...
def compute_surface_distances(mask_gt,
  ...
  # 通过卷积核的卷积操作，将每一个像素点与邻近7个点组成的立方体的邻近关系进行编码，对应0~255
  ...
  if WRITE_PNG:
    import cv2
    cv2.imwrite("borders_gt_kernel.png", neighbour_code_map_gt[5, :, :])
    cv2.imwrite("borders_pred_kernel.png", neighbour_code_map_pred[5, :, :])

  # create masks with the surface voxels
  # 将大于0的像素值，均变为1；同时要求，该像素值不为255；
  ...
  if WRITE_PNG:
    import cv2
    cv2.imwrite("borders_gt.png", borders_gt[5, :, :]*255)
    cv2.imwrite("borders_pred.png", borders_pred[5, :, :]*255)

  # compute the distance transform (closest distance of each voxel to the
  # surface voxels)
  # 距离转换，计算图像中非零点到最近背景点（即0）的距离，构造距离图
  if borders_gt.any():
  ...
  if WRITE_PNG:
    cv2.imwrite("borders_gt_dist.png", distmap_gt[5, :, :])
    cv2.imwrite("borders_pred_dist.png", distmap_pred[5, :, :])
  ...
  if WRITE_PNG:
    cv2.imwrite("borders_gt_map.png", surface_area_map_gt[5, :, :])
    cv2.imwrite("borders_pred_map.png", surface_area_map_pred[5, :, :])
